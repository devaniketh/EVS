...
export class HyperliquidPositionOpener {
   ...
   * @param depositAmount - Amount deposited in the vault (in ETH tokens)
   * @param coin - The coin to trade (default: 'ETH')
   * @param coinPrice - Current price of the coin in USD (optional, will fetch from CoinGecko if not provided)
   */
  async openPositionOnDeposit(
    depositAmount: number,
    coin: string = 'ETH',
    coinPrice?: number // Optional - will fetch from CoinGecko if not provided
  ) {
    try {
      console.log(`Opening position for deposit amount: ${depositAmount} ${coin}`);
      
      // Fetch current ETH price from CoinGecko if not provided
      let currentPrice = coinPrice;
      if (!currentPrice) {
        currentPrice = await this.fetchEthPriceFromCoinGecko();
      }
      
      // Ensure minimum deposit amount for meaningful position (0.01 ETH minimum)
      const MINIMUM_DEPOSIT_ETH = 0.01; // Minimum 0.01 ETH for position opening
      if (depositAmount < MINIMUM_DEPOSIT_ETH) {
        throw new Error(`Deposit amount ${depositAmount} ${coin} is below minimum required ${MINIMUM_DEPOSIT_ETH} ${coin} for Hyperliquid position opening`);
      }
      
      // Use the same ETH amount for the Hyperliquid position
      const coinAmount = depositAmount; // Same amount in ETH
      ...
      const limit_px = currentPrice * 1.0015; // Add 15 bps slippage
      const roundedLimitPx = Math.round(limit_px * 10) / 10; // Round to 1 decimal place for ETH

      console.log(`Calculated order parameters: price=${roundedLimitPx} (based on ETH price $${currentPrice}), size=${roundedSz}`);
  ...
  // ============================================================================
  // 6. PRICE FETCHING FROM COINGECKO
  // ============================================================================
...
// ============================================================================
// 8. HARDCODED TESTNET CONFIGURATION
// ============================================================================
